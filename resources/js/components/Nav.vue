<template>
  <header class="bg-white h-12 px-4 flex items-center border-b border-gray-400 shadow-sm">
    <div class="w-1/3">
      <div class="flex">
        ...
        <div class="ml-2 relative">
          ...
          <input
            type="text"
            name="search"
            class="rounded-full pl-8 pr-3 w-56 bg-gray-200 h-8 focus:outline-none focus:shadow-outline text-sm"
            placeholder="Search Facebook"
          />
        </div>
      </div>
    </div>
    <nav class="w-1/3 h-full">
      <ul class="flex justify-center items-center h-full">
        <li class="h-full">
          <router-link :to="{name:'home'}" class="block border-b-2 border-blue-500 px-6 h-full flex items-center">
